...
const asString = (value: FieldValue) => value == null ? undefined : String(value);
...
export default function CmsPage() {
  ...
  return <main className="min-h-screen bg-gray-50 text-gray-900">
    <div className="max-w-7xl mx-auto p-4 md:p-8">
      ...
      {tab === "brand" && brand && <section className="bg-white rounded-xl border p-4 grid md:grid-cols-2 gap-4"><Field label="Brand name" value={brand.brand_name} onChange={v=>setBrand({...brand,brand_name:asString(v)})}/><Field label="Primary domain" value={brand.primary_domain} onChange={v=>setBrand({...brand,primary_domain:asString(v)})}/><Field label="Logo URL" value={brand.logo_url} onChange={v=>setBrand({...brand,logo_url:asString(v)})}/><Field label="Favicon URL" value={brand.favicon_url} onChange={v=>setBrand({...brand,favicon_url:asString(v)})}/><Field label="Primary color" value={brand.primary_color} onChange={v=>setBrand({...brand,primary_color:asString(v)})}/><Field label="Secondary color" value={brand.secondary_color} onChange={v=>setBrand({...brand,secondary_color:asString(v)})}/><Field label="Background color" value={brand.background_color} onChange={v=>setBrand({...brand,background_color:asString(v)})}/><Field label="Text color" value={brand.text_color} onChange={v=>setBrand({...brand,text_color:asString(v)})}/><Field label="Footer description" value={brand.footer_description} onChange={v=>setBrand({...brand,footer_description:asString(v)})}/><Field label="Copyright text" value={brand.copyright_text} onChange={v=>setBrand({...brand,copyright_text:asString(v)})}/><button className="md:col-span-2 justify-self-start rounded-lg cms-primary-bg text-white px-4 py-2" onClick={()=>save(mwakwaData.brandSettings,brand)}>Save branding</button></section>}

      {tab === "business" && <section className="bg-white rounded-xl border p-4 grid md:grid-cols-2 gap-4">{business ? <><Field label="Legal name" value={business.legal_name} onChange={v=>setBusiness({...business,legal_name:asString(v)})}/><Field label="Trading name" value={business.trading_name} onChange={v=>setBusiness({...business,trading_name:asString(v)})}/><Field label="Registration number" value={business.registration_number} onChange={v=>setBusiness({...business,registration_number:asString(v)})}/><Field label="Tax ID" value={business.tax_id} onChange={v=>setBusiness({...business,tax_id:asString(v)})}/><Field label="Address line 1" value={business.address_line1} onChange={v=>setBusiness({...business,address_line1:asString(v)})}/><Field label="Address line 2" value={business.address_line2} onChange={v=>setBusiness({...business,address_line2:asString(v)})}/><Field label="City" value={business.city} onChange={v=>setBusiness({...business,city:asString(v)})}/><Field label="Region" value={business.region} onChange={v=>setBusiness({...business,region:asString(v)})}/><Field label="Postal code" value={business.postal_code} onChange={v=>setBusiness({...business,postal_code:asString(v)})}/><Field label="Country" value={business.country} onChange={v=>setBusiness({...business,country:asString(v)})}/><Field label="General email" value={business.email} onChange={v=>setBusiness({...business,email:asString(v)})}/><Field label="Phone" value={business.phone} onChange={v=>setBusiness({...business,phone:asString(v)})}/><Field label="Privacy email" value={business.privacy_email} onChange={v=>setBusiness({...business,privacy_email:asString(v)})}/><Field label="Support email" value={business.support_email} onChange={v=>setBusiness({...business,support_email:asString(v)})}/><Field label="Website" value={business.website} onChange={v=>setBusiness({...business,website:asString(v)})}/><Field label="Data controller name" value={business.data_controller_name} onChange={v=>setBusiness({...business,data_controller_name:asString(v)})}/><button className="md:col-span-2 justify-self-start rounded-lg cms-primary-bg text-white px-4 py-2" onClick={()=>save(mwakwaData.businessInfo,business)}>Save business information</button></> : <p className="md:col-span-2 text-gray-600">No Business Information record exists yet. Create the initial company record before launch; public legal pages will then populate from this single source of truth.</p>}</section>}
