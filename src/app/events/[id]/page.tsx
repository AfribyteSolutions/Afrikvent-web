      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="bg-white rounded-lg shadow-sm overflow-hidden">
          ...
          <div className="p-8">
            ...
            {(event.refund_policy_snapshot || event.cancellation_policy_snapshot) && (
              <div className="bg-white rounded-2xl shadow-sm p-6 mb-6">
                <h2 className="text-xl font-semibold text-gray-900 mb-4">Ticket & event policies</h2>
                <div className="space-y-4 text-sm">
                  {event.refund_policy_snapshot && <div><div className="font-semibold text-gray-900">{event.refund_policy_snapshot.name}</div><p className="text-gray-600 mt-1">{event.refund_policy_snapshot.buyer_disclosure || event.refund_policy_snapshot.description}</p></div>}
                  {event.cancellation_policy_snapshot && <div><div className="font-semibold text-gray-900">Cancellation & postponement</div><p className="text-gray-600 mt-1">{event.cancellation_policy_snapshot.buyer_disclosure || event.cancellation_policy_snapshot.description}</p></div>}
                </div>
              </div>
            )}
